...
export class ZapOutService {
  ...
  public async getZapOutRoute(zapOutRouteBodyDto: GetZapOutRouteBodyDto) {
    try {
      ...
      await this.validateTokenToSwap(zapOutRouteBodyDto, tokensWithCoingecko);

      const amounts = await this.determineTokenAmounts(zapOutRouteBodyDto, tokensWithCoingecko);
  ...
  private async validateTokenToSwap(
    dto: GetZapOutRouteBodyDto,
    tokens: ITokenPair & {
  ...
  private async determineTokenAmounts(
    ...
    try {
      await this.validateSharesToBurnAmount(dto);

      const tokenToSwapLower = dto.tokenToSwap.toLowerCase();
      const token0Lower = tokens.token0.address.toLowerCase();

      return await this.calculateTokenAmounts(dto, tokens, tokenToSwapLower === token0Lower);
    } catch (error) {
  ...
  private async validateSharesToBurnAmount(dto: GetZapOutRouteBodyDto): Promise<void> {
    if (!dto.sharesToBurn) {
      throw new BadRequestException("Missing token input amount");
    }

    try {
      const amount = BigInt(dto.sharesToBurn);
      if (amount <= 0n) {
  ...
  private async calculateTokenAmounts(
    ...
    try {
      if (isToken0Input) {
        const amount0Max = BigInt(dto.tokenToSwap);
        ...
        const amount1Max = BigInt(dto.tokenToSwap);
